            ...
            <template #main>
                ...
                <div class="flex flex-col items-center animate-fade-up">
                    <div v-if="salesBackups.length<1" class="flex flex-col items-center gap-1 pb-5">
                        ...
                        <p>Aqui se mostrará el resumen histórico de ventas cada que se guarden los datos en el boton
                            'guardar' en la seccion de <RouterLink :to="{name: 'sales'}"
                                class="text-lg font-bold underline text-sky-800">ventas</RouterLink>
                        </p>
                    </div>
                    <div v-if="salesBackups.length>0" class="flex items-center justify-around w-full my-2">
                        ...
                        <div>
                            <p @click="toggleShowExpenses" v-if="!showExpenses"
                    ...
                    <div class="flex items-center justify-end w-full px-2 py-2" >
                        <div class="flex flex-wrap items-center justify-center gap-1 px-2 text-white bg-sky-800 rounded-xl">
                                        <v-icon name="fa-chart-pie" scale="1.2" color="white" />
                                        <p class="font-medium">Mostrar gráfica</p>
                                        <div @click="toggleShowChart" class="">
                                            <div :class="{ 'bg-sky-200': !isToggled, 'bg-white': isToggled }"
                                                class="cursor-pointer transition-colors duration-500 ease-in-out box-border flex items-center w-7 h-4 px-[2px]  rounded-full shadow-sm">
                                                <div :class="{ 'translate-x-full': isToggled, 'translate-x-0': !isToggled }"
                                                    class="w-3 h-3 transition-transform duration-500 ease-in-out rounded-full bg-sky-800">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <section class="flex w-full justify-evenly">
                        ...
                        <div :class="{'w-full': !showExpenses}" class="flex flex-col min-w-[55%] ">
                            <div v-for="(arr) in salesBackups" :key="arr"
                                class="flex flex-col gap-1 p-1 py-1 mb-4 overflow-auto bg-gray-200 shadow-md full max-h-96 min-h-96 rounded-xl">
                                <div
                                    class="flex flex-col items-center justify-center gap-1 bg-white shadow-md rounded-xl">
                                    ...
                                    <div class="flex flex-col items-center gap-1">
                                        ...
                                        <p class="p-1 font-bold text-white bg-red-600 rounded-md">Gastos: ${{
                                            totalFinancialHistory[salesBackups.indexOf(arr)].totalExpenses}}</p>
                                        <p class="p-1 font-bold text-white bg-green-800 rounded-md">Balance: $ {{
                                            totalFinancialHistory[salesBackups.indexOf(arr)].finalBalance}}</p>
                                    </div>
                               
                                    <PieChart v-show="isToggled" :dataSet="[
                                ...
                                <div v-for="(elm) in arr" :key="elm" class="px-3 py-2 shadow-md bg-slate-50 rounded-xl">
                                    ...
                                    <p
                                        class="inline-block p-1 mb-2 font-bold text-white shadow-md bg-sky-800 rounded-xl">
                                        ${{elm.find((x) => x.itemDate).grandTotal}}</p>
                                    <div v-for="(item) in elm" :key="item" class="flex flex-wrap">
                                        <p v-if="item.itemName && item.itemName !== 'Otros'"
                                            class="inline-block p-1 mb-1 font-bold transition-transform bg-white shadow-md cursor-pointer text-sky-900 rounded-xl hover:scale-105 hover:border hover:border-sky-900">
                                            {{ item.itemAmount }} {{ item.itemName }} de ${{ item.itemPrice }} (${{
                                            item.itemSubtotal }})</p>
                                        <p v-if="item.itemName && item.itemName === 'Otros'"
                                            class="inline-block p-1 mb-1 font-bold bg-white shadow-md text-sky-900 rounded-xl hover:scale-105 hover:border hover:border-sky-900-">
                                            {{ item.itemName }} (${{ item.itemSubtotal }})</p>
...
// function to toggle the slider for the pie chart
